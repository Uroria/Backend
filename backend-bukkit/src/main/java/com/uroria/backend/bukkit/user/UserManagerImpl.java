...
public final class UserManagerImpl extends AbstractUserManager implements UserManager {
    ...
    public UserManagerImpl(PulsarClient pulsarClient, Logger logger) {
    ...
    protected void checkUser(@NonNull User user) {
        ...
        if (user.isDeleted()) {
            ...
            BukkitUtils.callAsyncEvent(new UserUpdateEvent(user));
            return;
        }
        ...
        for (User cachedUser : this.users) {
            ...
            BukkitUtils.callAsyncEvent(new UserUpdateEvent(cachedUser));
            return;
        }
    ...
    private void runCacheChecker() {
        BackendScheduler.runTaskLater(() -> {
            ...
            for (User user : this.users) {
                if (Bukkit.getPlayer(user.getUUID()) == null) markedForRemoval.add(user.getUUID());
            }
