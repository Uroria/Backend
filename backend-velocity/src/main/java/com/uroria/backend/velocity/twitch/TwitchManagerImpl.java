...
public final class TwitchManagerImpl extends AbstractTwitchManager implements TwitchManager {
    ...
    public TwitchManagerImpl(PulsarClient pulsarClient, Logger logger, ProxyServer proxyServer) {
    ...
    protected void checkSubscriber(@NonNull Subscriber subscriber) {
        ...
        for (Subscriber cachedSubscriber : this.subscribers) {
            ...
            this.proxyServer.getEventManager().fireAndForget(new SubscriberUpdateEvent(cachedSubscriber));
            return;
        }

        logger.info("Adding " + subscriber);
        this.subscribers.add(subscriber);
        this.proxyServer.getEventManager().fireAndForget(new SubscriberUpdateEvent(subscriber));
    }
    ...
    protected void checkStreamer(@NonNull Streamer streamer) {
        ...
        for (Streamer cachedStreamer : this.streamers) {
            ...
            this.proxyServer.getEventManager().fireAndForget(new StreamerUpdateEvent(cachedStreamer));
            return;
        }

        logger.info("Adding " + streamer);
        this.streamers.add(streamer);
        this.proxyServer.getEventManager().fireAndForget(new StreamerUpdateEvent(streamer));
    }
