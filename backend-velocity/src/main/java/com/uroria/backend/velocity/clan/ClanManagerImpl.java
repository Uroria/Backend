...
public final class ClanManagerImpl extends AbstractClanManager implements ClanManager {
    ...
    public ClanManagerImpl(PulsarClient pulsarClient, Logger logger, ProxyServer proxyServer) {
    ...
    protected void checkClan(@NonNull Clan clan) {
        ...
        if (clan.isDeleted()) {
            ...
            this.proxyServer.getEventManager().fireAndForget(new ClanUpdateEvent(clan));
            return;
        }
        ...
        for (Clan cachedClan : this.clans) {
            ...
            this.proxyServer.getEventManager().fireAndForget(new ClanUpdateEvent(cachedClan));
            return;
        }
        ...
        BackendScheduler.runTaskLater(() -> {
            ...
            for (Clan clan : this.clans) {
                ...
                for (UUID uuid : clan.getMembers()) {
                    if (this.proxyServer.getPlayer(uuid).isPresent()) {
                        remove = false;
                        break;
                    }
                }
