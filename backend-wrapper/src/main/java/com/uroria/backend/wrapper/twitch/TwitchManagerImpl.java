package com.uroria.backend.wrapper.twitch;
...
public final class TwitchManagerImpl extends AbstractTwitchManager implements TwitchManager {
    ...
    public TwitchManagerImpl(PulsarClient pulsarClient, Logger logger, Function<UUID, Boolean> onlinePlayerCheck, boolean offline, EventManager eventManager) {
    ...
    protected void checkSubscriber(@NonNull Subscriber subscriber) {
        ...
        for (Subscriber cachedSubscriber : this.subscribers) {
            ...
            this.eventManager.callAndForget(new SubscriberUpdateEvent(cachedSubscriber));
            return;
        }

        logger.info("Adding " + subscriber);
        this.subscribers.add(subscriber);
        this.eventManager.callAndForget(new SubscriberUpdateEvent(subscriber));
    }
    ...
    protected void checkStreamer(@NonNull Streamer streamer) {
        ...
        for (Streamer cachedStreamer : this.streamers) {
            ...
            this.eventManager.callAndForget(new StreamerUpdateEvent(cachedStreamer));
            return;
        }

        logger.info("Adding " + streamer);
        this.streamers.add(streamer);
        this.eventManager.callAndForget(new StreamerUpdateEvent(streamer));
    }
